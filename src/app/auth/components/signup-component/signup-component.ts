...
export class SignupComponent {
  ...
  protected signupForm = this._formBuilder.group({
    name: this._formBuilder.nonNullable.control('testasdfasdf', [
      Validators.required,
      Validators.minLength(6),
    ]),
    email: this._formBuilder.nonNullable.control('test@test.company', [
      Validators.required,
      Validators.email,
    ]),
    password: this._formBuilder.nonNullable.control('asdQSDFKFJf@a1341', [Validators.required]),
    passwordConfirmation: this._formBuilder.nonNullable.control('asdQSDFKFJf@a1341', [
      Validators.required,
    ]),
  ...
  protected signup(signupData: ISignUpData) {
    if (this.signupForm.controls.agreeToS.value === false) {
